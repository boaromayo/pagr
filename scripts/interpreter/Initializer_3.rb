...
class Interpreter
  ...
  def command_101
    ...
    if $game_temp.in_battle
      allowed_line = 0
      $game_temp.battle_message_text.push(@list[@index].parameters[0])
    else
      $game_temp.message_text = @list[@index].parameters[0] + "\n"
      if $game_temp.message_text.include?('\x')
        allowed_line = 5
      else
        allowed_line = 4
      end
    ...
    loop do
      if @list[@index+1].code == 401
        if $game_temp.in_battle
          $game_temp.battle_message_text.push(@list[@index+1].parameters[0])
        else
        ...
        if @list[@index+1].code == 102
          if @list[@index+1].parameters[0].size <= allowed_line - line_count
            @index += 1
  ...
  def command_111
    ...
    when 4
      ...
      if actor != nil
        ...
        when 1
          n1 = actor.name.downcase
          n2 = @parameters[3].downcase
          result = (n1 == n2)
        when 2
